...
app.get('/admin/properties/:id', requireAuth, (req, res) => {
  ...
  const categoryBlocks = CATEGORIES.map((cat) => {
    ...
    return `<div class="category-block">
      ...
      <div class="add-forms">
        <form method="POST" action="/admin/properties/${property.id}/upload" enctype="multipart/form-data" class="inline-form">
...
app.post('/admin/properties/:id/upload', requireAuth, upload.single('file'), (req, res) => {
  const data = loadData();
  const property = data.properties.find((p) => p.id === req.params.id);
  if (!property || !req.file) return res.redirect('/admin');
